...
def main(args):
    ...
    DLS = MedDataHelpers.getLoaders(chexpert_dat, args)
    DL = DLS[subset[0]]

    query_embeddings = CLIP_Embedding.getLabelEmbeddings(clip_model, heads, avg = False)
    candidate_embeddings, candidate_labels = utils.get_all_preds(DL, clip_model, im_embeds=True, heads = heads)
    query_labels, candLabels = getCandidates(query_embeddings, candidate_embeddings[0], candidate_labels, heads)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--je_model_path', type=str, default='/n/data2/hms/dbmi/beamlab/anil/Med_ImageText_Embedding/models/clip_regularized/exp4/', help='path for saving trained models')
