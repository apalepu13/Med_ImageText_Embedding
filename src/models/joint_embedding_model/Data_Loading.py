...
class Image_Text_Dataset(Dataset):
    ...
    def __init__(self, source = 'mimic_cxr', group='train',
        ...
        if self.source == 'm':
            ...
            self.source = 'covid-chestxray'
        elif self.source == 'med':
            self.source = 'medpix'
        ...
        if self.source == 'mimic_cxr':
            ...
            if group == 'tinyval':
                group = 'val'
                self.im_list = self.im_list.iloc[::30, :]
            ...
            #pd.read_csv(chexpert_root_dir + 'convirt-retrieval/image-retrieval/query.csv')
            elif group == 'all':
            ...
            self.root_dir = covid_chestxray_csv_file
            self.im_list = pd.read_csv(self.root_dir)
            self.im_list['Pneumonia'] = self.im_list['label'].str.contains('Pneumonia')
            self.im_list['No Finding'] = self.im_list['label'].str.contains('No Finding')
            self.im_list['covid19'] = self.im_list['label'].str.contains('covid19')
    ...
    def __getitem__(self, idx):
        ...
        if self.source == 'mimic_cxr':
            ...
            image1_orig = self.im_preprocessing_train(image)
            image2_orig = self.im_preprocessing_train(image)
            if self.synth:
                ...
                #print("Images synthesized")
            else:
                image1 = self.im_finish(image1_orig)
                image2 = self.im_finish(image2_orig)
