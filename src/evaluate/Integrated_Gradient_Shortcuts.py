...
def main(args):
    ...
    finetunedname = 'frozen_finetunedv4_best_model.pt'
    clip_normal = CLIP_Embedding.getCLIPModel(modelpath=args.je_model_path, modname=finetunedname, freezeText=True, freezeCNNEncoder=False, eval=False)
    clip_synth = CLIP_Embedding.getCLIPModel(modelpath=args.je_model_path_synth, modname=finetunedname, freezeText=True, freezeCNNEncoder=False, eval=False)
    cnn_normal = Vision_Model.getCNN(loadpath=args.vis_model_path, loadmodel=finetunedname, freeze=False, eval=False)
    cnn_synth = Vision_Model.getCNN(loadpath=args.vis_model_path_synth, loadmodel=finetunedname, freeze=False, eval=False)

    clip_normal = CLIP_Embedding.SimClassifier(heads = heads1, clip_model = clip_normal)
    clip_synth = CLIP_Embedding.SimClassifier(heads=heads1, clip_model=clip_synth)
    ...
    dat2 = MedDataHelpers.getDatasets(source=args.sr, subset=[args.subset], heads=heads1, filters=filters, synthetic=True, get_overwrites=True)

    baselinesample = dat[args.subset].__getitem__(1)
    baselinesample = baselinesample['images'][0]
    baselinesample = baselinesample[None, :, :, :]
    cnn_norm_norms, cnn_norm_synths, cnn_synth_norms, cnn_synth_synths, clip_norm_norms, clip_synth_synths, clip_norm_synths, clip_synth_norms = [], [], [], [], [], [], [], []
    for i in range(234):
        for j, myhead in enumerate(heads1):
            sample = dat[args.subset].__getitem__(i)
            sample2 = dat2[myhead].__getitem__(i)
            image1 = sample['images']
            image2 = sample2['images']
            for k, im in enumerate(image1):
                image1[k] = im[None, :, :, :]
            for k, im in enumerate(image2):
                image2[k] = im[None, :, :, :]
            sample['images'] = image1
            sample2['images'] = image2

            fig, ax = plt.subplots(2, 5, figsize=(16, 7))
            cnn_synth_synths.append(plotAttributions(sample2, ax[1, 4], cnn_synth, myhead, name="Synth CNN"))
            clip_norm_norms.append(plotAttributions(sample, ax[0, 1], clip_normal, myhead, name="Real CLIP"))
            clip_norm_synths.append(plotAttributions(sample2, ax[1, 1], clip_normal, myhead, name="Real CLIP"))
            clip_synth_norms.append(plotAttributions(sample, ax[0, 2],clip_synth, myhead, name="Synth CLIP"))
            clip_synth_synths.append(plotAttributions(sample2, ax[1, 2], clip_synth, myhead, name="Synth CLIP"))
            cnn_norm_norms.append(plotAttributions(sample, ax[0, 3], cnn_normal, myhead, name="Real CNN"))
            cnn_norm_synths.append(plotAttributions(sample2, ax[1, 3], cnn_normal, myhead, name="Real CNN"))
            cnn_synth_norms.append(plotAttributions(sample, ax[0, 4], cnn_synth, myhead, name="Synth CNN"))

            plot_original_image(sample, ax[0, 0], heads1, index=i, name=myhead + " gradient")
            plot_original_image(sample2, ax[1, 0], heads1, index=i, name=myhead + " gradient, watermarked")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--je_model_path_synth', type=str, default='/n/data2/hms/dbmi/beamlab/anil/Med_ImageText_Embedding/models/synth_clip_regularized/exp2/', help='path for saving trained models')
